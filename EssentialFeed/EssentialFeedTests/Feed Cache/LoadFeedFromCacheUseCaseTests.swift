...
class LoadFeedFromCacheUseCaseTests: XCTestCase {
    ...
    func test_load_deliversNoImagesOnCacheExpiration() {
        let feed = uniqueImageFeed()
        let fixedCurrentDate = Date()
        let expirationTimestamp = fixedCurrentDate.minusFeeCacheMaxAge()
        let (sut, store) = makeSUT(currentDate: { fixedCurrentDate })
    ...
    func test_load_doesNoSideEffectsOnEmptyCache() {
        let (sut, store) = makeSUT()
    ...
    func test_load_hasNoSideAffectsOnExpiredCache() {
        let feed = uniqueImageFeed()
        let fixedCurrentDate = Date()
        let expiredTimestamp = fixedCurrentDate.minusFeeCacheMaxAge().adding(days: -1)
        let (sut, store) = makeSUT(currentDate: { fixedCurrentDate })
    ...
    private func expect(_ sut: LocalFeedLoader,
                        ...
                        file: StaticString = #filePath,
                        line: UInt = #line) {
        let exp = expectation(description: "Wait for load command")

...
class CodableFeedStoreTests: XCTestCase {
    ...
    func test_retrieve_deliversFailureOnRetrievalError() {
        ...
        try! "invalid data".write(to: testSepcificStoreURL(), atomically: false, encoding: .utf8)
    ...
    private func insert(_ cache: (feed: [LocalFeedImage], timestamp: Date), to sut: CodableFeedStore, file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.insert(cache.feed, timestamp: cache.timestamp) { insertionError in
            XCTAssertNil(insertionError, "Expected to be inserted successfully", file: file, line: line)
            exp.fulfill()
        }
        wait(for: [exp], timeout: 1.0)
    }
